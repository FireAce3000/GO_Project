...
func main() {
	...
	// defer - führt den Befehl am schluss aus, wenn alles andere ausgeführt wurde
	...
	switch {
	case input < 100:
		fmt.Println("Number is bigger than 100")
	case input == 100:
		fmt.Println("Number is 100")
	case input < 100:
		fmt.Println("number is lower than 100")
	}
	...
	fmt.Print("What should you do with whis number? [1] add 3 [2] sub 3 :")
	fmt.Scan(&input2)

	// es gibt keine while! while wird mit der for gemacht 
	for input2 != 0 {
		switch input2 {
			case 0:
				fmt.Println("end")
